...
const Header = () => {
  ...
  const handleClick = () => { setClick(!click); setAccount(false) }
  const closeMobileMenu = () => setClick(false)

  const [account, setAccount] = useState(false)
  const handleAccount = () => { setAccount(!account); setClick(false) }
      ...
      <nav className='bg-black sticky top-0 z-50 w-full py-6 sm:px-4 md:px-16 leading-relaxed'>
        ...
        <div className='flex text-white justify-between relative'>
          ...
          <div className="sm:hidden">
            {
              click && (<ul className='w-full absolute p-1 z-50 left-0 top-7 bg-green-800 mt-6 flex flex-col text-center sm:hidden'>
...

          {
            token
              ? <>
                ...
                {
                  account && (<ul className='max-w-full min-w-60 p-3 absolute z-50 top-8 sm:top-7 ease-in-out right-1 sm:-right-2 md:-right-4 rounded-lg bg-[#186366] mt-6 flex flex-col'>
              ...
              : <>
                ...
                <div className='text-xl absolute right-0 pr-2 hover:cursor-pointer mt-1 sm:hidden' onClick={handleAccount}>
                  {
                    account ? <IconContext.Provider value={{ color: "white" }}><BsBackspaceReverseFill /></IconContext.Provider> : <IconContext.Provider value={{ color: "white", size: "1.5rem" }}><MdAccountCircle /></IconContext.Provider>
                  }
                </div>
                {
                  account && (<ul className='w-36 p-1 absolute z-50 top-8 ease-in-out right-1 rounded-md bg-green-800 mt-6 flex flex-col text-center sm:hidden'>
                    <li className='text-base my-2 pt-[2px] inline-block hover:scale-110  hover:font-semibold'>
                      <Link to='/login' onClick={closeMobileAccount}>
                        Login
                      </Link>
                    </li>
                    <li className='text-base my-2 pb-[2px] inline-block hover:scale-110  hover:font-semibold'>
                      <Link to='/register' onClick={closeMobileAccount}>
                        Register
                      </Link>
                    </li>
                  </ul>)
