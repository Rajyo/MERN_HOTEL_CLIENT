...
const Room = () => {
  ...
  const discount = 10
  const specialNote = "Check-in time is 12:00 PM, checkout time is 11:59 AM. If you leave behind any items, please contact the receptionist"
  ...
  const handleBookNowClick = async (e) => {
    ...
    try {
      const { data: session } = await axiosInstance.post('api/bookings/create-checkout-session', formData);
            ...
            <div>
              ...
              <div className='flex my-11 justify-between gap-4 flex-wrap'>
                {data?.amenities?.slice(0, 4)?.map(amenity => (
                  <div key={amenity} className='md:w-40 w-fit text-center px-2 md:px-0 h-20 md:h-36 bg-[#eff0f2] dark:bg-gray-800 rounded-lg grid place-content-center'>
              ...
              <div className='mb-11'>
                ...
                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. {data.description} Quos maxime voluptate magnam eaque, nam cupiditate eum ipsa! Molestias omnis maiores repellat eos aperiam velit natus quaerat! Modi fugit minus perferendis.</p>
              </div>
              ...
              <div className='mb-14'>
                ...
                <div className='grid grid-cols-2 lg:grid-cols-3 gap-2'>
                  {data.amenities.map(amenity => (
                    <div key={amenity} className='flex justify-between md:my-0 my-1' >
              ...
              <section className='mb-11'>
                ...
                <div className='grid grid-cols-2'>
                  <div className='flex items-center my-1 md:my-0'>
                    ...
                    <p className='ml-2 max-[500px]:text-xs min-[500px]:text-sm sm:text-base'>Daily Cleaning</p>
                  </div>
                  <div className='flex items-center my-1 md:my-0'>
                  ...
                  <div className='flex items-center my-1 md:my-0'>
                    <GiSmokeBomb />
                    <p className='ml-2 max-[500px]:text-xs min-[500px]:text-sm sm:text-base'>Smoke Detectors</p>
                  </div>
                </div>
              </section>

              <div className='mb-14'>
                <h2 className='font-bold text-3xl mb-4 break-words'>Complementary</h2>
                <div className='grid grid-cols-2 lg:grid-cols-3 gap-2'>
            ...
            <div className='px-4 py-6 '>
              <h3 className='flex flex-wrap justify-center'>
                <span className={`${discount ? 'text-gray-400' : ''} font-bold text-xl`} >
                  <span className='flex'><span className='mt-1'><FaRupeeSign /></span>{price}</span>
                </span>
                {discount ? (
                  <span className='font-bold text-xl flex ml-2'>
                    Now at
                    <span className='text-[#f27405] ml-2'>
                      <span className='flex'><span className='mt-1'><FaRupeeSign /></span>{Math.round(discountPrice)}</span>
                    </span>
                  </span>
                ) : (
                  ''
              ...
              <div className='flex'>
                <div className='w-1/2 pr-2'>
                  ...
                  <DatePicker id='check-in-date' selected={checkinDate} onChange={date => setCheckinDate(date)} dateFormat='dd/MM/yyyy' minDate={new Date()} className='w-full border text-black border-gray-300 rounded-lg p-2.5 focus:ring-primary focus:border-primary' />
                </div>
                <div className='w-1/2 pl-2'>
                  <label htmlFor='check-out-date' className='block text-sm font-medium text-gray-900 dark:text-gray-400'>
                    Check Out date
                  </label>
                  <DatePicker id='check-out-date' minDate={calcMinCheckoutDate()} selected={checkoutDate} onChange={date => setCheckoutDate(date)} dateFormat='dd/MM/yyyy' disabled={!checkinDate} className='w-full border text-black border-gray-300 rounded-lg p-2.5 focus:ring-primary focus:border-primary' />
                </div>
              </div>
              ...
              {calcNoOfDays() > 0 ? (
                <p className='flex justify-center mt-6 -mb-3 text-white rounded-md py-2 bg-orange-500 font-semibold'>
                  Total Price : <span className='flex ml-2'><span className='mt-1'><FaRupeeSign /></span>{Math.round(calcNoOfDays() * discountPrice)}</span></p>
              ) : (
                <></>
              )}

              <button onClick={handleBookNowClick} disabled={isDisabled} className='bg-black text-white py-2 rounded-md w-full mt-6 disabled:bg-gray-500 disabled:cursor-not-allowed hover:scale-105'
              >
                Book Now
              </button>
            </div>
